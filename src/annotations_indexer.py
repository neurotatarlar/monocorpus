...
def sync_annotations():
    ...
    for f, md5 in track(downloaded_files, description="Processing downloaded annotations..."):
        with open(f, 'r') as file:
            ...
            if not (doc := docs_cache.get(doc_md5)):
                doc = Document.first(formula=f"md5='{doc_md5}'")
                docs_cache[doc_md5] = doc
    ...
    for doc in not_completed_docs:
        ...
        if completeness == 1.0:
            ...
            upload_results = upload_files_to_s3(
                [output_file], bucket_provider=lambda c: c['yc']['bucket']['annotations_summary'], session=session
            )
            ...
            anno_sum.missing_pages = missing_pages.__str__()
