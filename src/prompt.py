...
def cook_extraction_prompt(batch_from_page, batch_to_page, next_footnote_num, headers_hierarchy, source_path, result_path):
   ...
   prompt = [EXTRACT_CONTENT_PROMPT_HEADER.format(_to=batch_to_page, _from=batch_from_page, next_footnote_num=next_footnote_num, headers_hierarchy=headers_hierarchy)]
   prompt.append(f"Maintain global sequential numbering for footnotes starting from {next_footnote_num}: [^{next_footnote_num}]")
   if batch_from_page:
      prompt.append(EXTRACT_CONTENT_PROMPT_NO_TITLE.format(headers_hierarchy=headers_hierarchy))
   else:
      prompt.append(EXTRACT_CONTENT_PROMPT_POSSIBLE_TITLE)
   prompt.append(f"Now, extract structured content from the following document @{source_path} and save the output to @{result_path}.")
   return "\n".join(prompt).strip()

# def _cook_extraction_prompt(batch_from_page, batch_to_page, next_footnote_num, headers_hierarchy, source_path, result_path):
#    if headers_hierarchy:
#       headers_hierarchy = "\n".join(headers_hierarchy)
#       headers_hierarchy =  f"headers_hierarchy = ```\n{headers_hierarchy}\n```"
#    else:
