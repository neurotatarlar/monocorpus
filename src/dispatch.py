...
def layout_analysis_entry_point(md5, force, pages_slice):
    ...
    files = {calculate_md5(file): file for file in files}

    # if user specified the MD5 of the document
    if md5:
        # if the document with the provided MD5 is not in the local folder, then download it from Yandex.Disk
        if md5 in files:
            file = files[md5]
        else:
            file = os.path.join(get_path_in_workdir(Dirs.ENTRY_POINT), f"{md5}.pdf")
            doc = Document.first(
                fields=[Document.ya_public_key.field_name, Document.sent_for_annotation.field_name],
                formula=f"md5='{md5}'"
            )
            if doc and (not doc.sent_for_annotation or force):
                print(f"Document with MD5 `{md5}` not found in the local folder, downloading it from Yandex.Disk")
                download_file_from_yandex_disk(doc.ya_public_key, file)
            elif doc and doc.sent_for_annotation:
                print(f"Document with MD5 `{md5}` already sent for annotation. Skipping...")
                return
            else:
                print(f"Document with MD5 `{md5}` not found in the local folder and in the Airtable")
                return
        files = {md5: file}
