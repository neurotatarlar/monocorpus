...
class AnnotationSummary(Model):
    ...
    def get_or_create(doc_md5):
        if not (summary := AnnotationSummary.first(formula=f"{AnnotationSummary.doc_md5.field_name}='{doc_md5}'")):
            summary = AnnotationSummary(doc_md5=doc_md5)
        return summary
