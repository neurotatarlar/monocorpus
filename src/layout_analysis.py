...
def layout_analysis(path_to_file: str, md5: str, pages_slice):
    ...
    title_task = tasks_paths.pop(0, None)
    if title_task:
        upload_files_to_s3(
            [title_task],
...
def _semantic_transform(page_layout, page_no, path_to_file):
    ...
    for idx, l in enumerate(sorted_layout):
        ...
        elif idx > 0 and this_class == 'page-header' and prev_class != 'page-header':
            this_class = "text"
        # replace all `list-item` with `text` because there are a lot of errors on this class
        elif this_class == 'list-item':
            this_class = "text"
        # if this is first element on the page, and it is a `section-header` then replace it with `page-header`,
        elif idx == 0 and this_class == 'section-header':
        ...
        for block in p.get_text("dict", clip=b, sort=True).get("blocks", []):
            for line in block.get("lines", []):
                for span in line.get("spans", []):
                    ...
                    if text and text.isnumeric() and not prev_span:
        ...
        for l in reversed(sorted_layout):
            match l["class"]:
                ...
                case "text" | "page-footer":
                    page = doc[page_no]
                    width_ratio = page.rect.width / page_layout['width']
                    height_ratio = page.rect.height / page_layout['height']
                    bbox = l["bbox"]
                    x1 = bbox[0] * width_ratio
                    y1 = bbox[1] * height_ratio
                    x2 = bbox[2] * width_ratio
                    y2 = bbox[3] * height_ratio
                    bbox = (x1, y1, x2, y2)
