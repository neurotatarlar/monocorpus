    ...
    def _triplets(_dir, _prelude):
        for idx, ground_truth_path in enumerate(_list_files(_dir, enswith='.md'), start=1):
            ...
            with open(ground_truth_path, 'r') as f:
                prompt.append({"text": f"✅ Example {idx+1} Ground Truth in Markdown format:\n{f.read()}"})
            
    _triplets('./shots/paragraphs', "Example {idx} Paragraph continued on the next page:")
    prompt.append({"text": "Summary: **Do not** insert a blank line if the paragraph is continuing. Merge seamlessly and naturally"})    

    _triplets('./shots/tables', "Example {idx} Table continued on the next page:")
